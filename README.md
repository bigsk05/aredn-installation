...
optional rootfs expansion → automatic reboot. Built for KVM panels that can only mount an installer
...
installer/                        # Go interactive installer (static binary)
  main.go                         #   orchestration: boot-mode detect / image pick / confirm / write / reboot
...
3. After the write completes the machine **reboots automatically after 15 seconds** (press any key + Enter to
   cancel and stay in the installer) → **detach the ISO in the panel** so it boots from disk.
