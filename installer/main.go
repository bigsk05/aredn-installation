...
//  5. powering off automatically when done.
//
// Build (static, no CGO):
//
...
func main() {
	...
	for _, d := range disks {
		fmt.Printf("  Candidate: %-12s %10s  %s\n", d.Path, humanSize(d.Size), d.Model)
	}
...
// finish prints the summary and powers off after 15 seconds (any key + Enter cancels and keeps it running).
func finish() {
	fmt.Println("\n==================================================")
	fmt.Println("AREDN firmware has been written.")
	fmt.Println("1) Detach/unmount this ISO in your provider panel;")
	fmt.Println("2) Make sure the machine boots from disk;")
	fmt.Println("3) After boot, the node defaults to 192.168.1.1 (localnode.local.mesh).")
	fmt.Println("==================================================")
	fmt.Println("(Powering off automatically in 15 seconds; press any key + Enter to cancel and keep running)")
	...
	select {
	case <-cancel:
		fmt.Println("Staying up. Run 'poweroff' anytime.")
...
// runPoweroff tries several shutdown methods in order until the system starts shutting down.
func runPoweroff() {
	cmds := [][]string{
		{"/sbin/poweroff"}, // busybox poweroff, graceful shutdown (verified working)
		{"poweroff"},
		{"/sbin/reboot", "-p", "-f"},
		{"reboot", "-p", "-f"},
	}
	for _, c := range cmds {
		if err := exec.Command(c[0], c[1:]...).Run(); err == nil {
			select {} // system is shutting down; block until power-off
		}
	}
	// last resort: kernel sysrq hard power-off
	_ = os.WriteFile("/proc/sysrq-trigger", []byte("o"), 0644)
	select {}
}
